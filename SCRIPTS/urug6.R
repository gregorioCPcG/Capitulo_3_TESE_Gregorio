...
df <- read_csv("df.csv")
df <- df %>%
  filter(S002VS == 6 & S003==858)#urug6
...
#tem muito NA achei estranho abri mão
...
summary(df)
#modelos
logitFrenteAmplio__1 <- glm(FrenteAmplio_~Gender+region+raca_branc+
                              idade_faixa+
...
logitFrenteAmplio__3 <- glm(FrenteAmplio_~autoloc_dir_esq+interesse+escolaridade_niveis+
                              ...
                              desconfia+promercado,data = df, family=binomial(link=logit))
logitFrenteAmplio__4 <- glm(FrenteAmplio_~Gender+region+raca_branc+
                              idade_faixa+
...
library(nnet)
modeloMulti <- multinom(df_voto_multinom ~ Gender+region+raca_branc+
                          idade_faixa+
                          status_emplo+rend_subj+religion+autoloc_dir_esq+interesse+escolaridade_niveis+
                          fundamentalism+antidemocr+
                          desconfia+promercado,
