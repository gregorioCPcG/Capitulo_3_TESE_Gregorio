...
df <- read_csv("df.csv")
df <- df %>%
  filter(S002VS == 3 & S003==604)#peru3
...
df$Cambio90_ <- df$E179WVS == 604008 
table(df$Cambio90_)
df$Cambio90_ <- as.numeric(df$Cambio90_)
df$Cambio90_ <- as.factor(df$Cambio90_)
table(df$Cambio90_)
prop.table(table(df$Cambio90_))
# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 604008 , "Cambio90_",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3,4),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("Cambio90_", "DK/NA/blank", "Others"))
...
print(df[, 12:25])
summary(df[,12:25])
imp <- mice(df[,12:25], seed=23109)# o nome da base e a seed sempre essa 23109
df[,12:25] <- complete(imp, 1)#sempre a m como  destino da mputação , empre escolher a 1
rm(imp)
summary(df[,12:25])
...
summary(df)
#modelos
logitCambio90__1 <- glm(Cambio90_~Gender+raca_branc+
                     idade_faixa+
                     status_emplo+rend_subj+religion,data = df, family=binomial(link=logit))
logitCambio90__2 <- glm(Cambio90_~fundamentalism+antidemocr+
                     desconfia+promercado,data = df, family=binomial(link=logit))
logitCambio90__3 <- glm(Cambio90_~autoloc_dir_esq+interesse+escolaridade_niveis+
                     fundamentalism+antidemocr+
                     desconfia+promercado,data = df, family=binomial(link=logit))
logitCambio90__4 <- glm(Cambio90_~Gender+raca_branc+
                     idade_faixa+
                     status_emplo+rend_subj+religion+autoloc_dir_esq+interesse+escolaridade_niveis+
                     fundamentalism+antidemocr+
                     desconfia+promercado,
                   data = df, family=binomial(link=logit))
tab_model(logitCambio90__1,logitCambio90__2,logitCambio90__3,logitCambio90__4,
...
# Definir "Cambio90_" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "Cambio90_")
...
library(lsr)
df$Cambio90_ <- as.numeric(df$Cambio90_)
model<- aov(Cambio90_ ~ Gender+raca_branc+
              idade_faixa+escolaridade_niveis+
...
df$votos_partidos <- ifelse(dfao$E179WVS < 5, "Nenhum/NSabe",
                            ifelse(dfao$E179WVS == 604001, "Perú Possible",
                                   ifelse(dfao$E179WVS == 604005, "Unión por el Perú",
                                          ifelse(dfao$E179WVS == 604002, "APRA",
                                                 ifelse(dfao$E179WVS == 604008, "Cambio 90",
                                                        "Outros Partidos")))))

# Definir a ordem dos níveis na coluna "votos_partidos"
df$votos_partidos <- factor(df$votos_partidos, levels = c("Perú Possible", "Unión por el Perú", "APRA", "Cambio 90", "Outros Partidos", "Nenhum/NSabe"))
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Peru Onda 3 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
