...
df <- read_csv("df.csv")
df <- df %>%
  filter(S002VS == 7 & S003==32)#argentina 7
...
df$ethnic_group <- factor(ifelse(df$X051 == 32001, "White",
                                 ifelse(df$X051 == 32002, "Light Brown","Other Brown")))

df$ethnic_group <- as.factor(df$ethnic_group)
levels(df$ethnic_group)
df$ethnic_group <- relevel(df$ethnic_group, ref = "White")
...
df$Unidad_Ciudadano <- df$E179WVS == 32001 
table(df$Unidad_Ciudadano)
df$Unidad_Ciudadano <- as.numeric(df$Unidad_Ciudadano)
df$Unidad_Ciudadano <- as.factor(df$Unidad_Ciudadano)
table(df$Unidad_Ciudadano)

# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 32001 , "Unidad_Ciudadano",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("Unidad_Ciudadano", "DK/NA/blank", "Others"))
...
df <- subset(df, select=c(X049,Gender,idade_faixa,
                          ...
                          Unidad_Ciudadano,df_voto_multinom,
                          E069_07,E069_12,E069_17,E069_06,F028,
                          E037,E040,E036,E116,E114,E117,F120,F118,E039,E235,rend_subj,region,ethnic_group))
summary(df[,1:10])
summary(df[,26:28])
df[,11:25]#issues
...
df <- subset(df, select=c(X049,Gender,idade_faixa,
                          escolaridade_niveis,status_emplo,religion,
                          autoloc_dir_esq, interesse,Unidad_Ciudadano,df_voto_multinom,
                          E069_07,E069_12,E069_17,E069_06,F118_invertido,
                          F120_invertido,F028_invertido,
                          E037,E040_invertido,E036_invertido,
                          E116_invertido,E114_invertido,E117,E039_invertido,E235_invertido,rend_subj,region,ethnic_group))
...
prop.table(table(df$Unidad_Ciudadano))
...
df <- subset(df, select=c(X049,Gender,idade_faixa,
                          escolaridade_niveis,status_emplo,religion,
                          autoloc_dir_esq, interesse,Unidad_Ciudadano,df_voto_multinom,
                          fundamentalism,desconfia,antidemocr,promercado,rend_subj,region,ethnic_group))
rm(dfao,fit)

#modelos
logitUnidad_Ciudadano_1 <- glm(Unidad_Ciudadano~X049+Gender+rend_subj+region+ethnic_group+
                              idade_faixa+escolaridade_niveis+
                              status_emplo+religion,data = df, family=binomial(link=logit))
logitUnidad_Ciudadano_2 <- glm(Unidad_Ciudadano~fundamentalism+antidemocr+
                              desconfia+promercado,data = df, family=binomial(link=logit))
logitUnidad_Ciudadano_3 <- glm(Unidad_Ciudadano~autoloc_dir_esq+interesse+
                              fundamentalism+antidemocr+
                              desconfia+promercado,data = df, family=binomial(link=logit))
logitUnidad_Ciudadano_4 <- glm(Unidad_Ciudadano~X049+Gender+rend_subj+region+ethnic_group+
                              idade_faixa+escolaridade_niveis+
                              status_emplo+religion+autoloc_dir_esq+interesse+
                              fundamentalism+antidemocr+
                              desconfia+promercado,
                            data = df, family=binomial(link=logit))
tab_model(logitUnidad_Ciudadano_1,logitUnidad_Ciudadano_2,logitUnidad_Ciudadano_3,logitUnidad_Ciudadano_4,
...
bic_modelo1 <- BIC(logitUnidad_Ciudadano_1)
bic_modelo2 <- BIC(logitUnidad_Ciudadano_2)
bic_modelo3 <- BIC(logitUnidad_Ciudadano_3)
bic_modelo4 <- BIC(logitUnidad_Ciudadano_4)
...
# Definir "Unidad_Ciudadano" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "Unidad_Ciudadano")

library(nnet)
modeloMulti <- multinom(df_voto_multinom ~ X049+Gender+rend_subj+region+ethnic_group+
                          idade_faixa+escolaridade_niveis+
                          status_emplo+religion+autoloc_dir_esq+interesse+
                          fundamentalism+antidemocr+
                          desconfia+promercado,
...
logitfundamentalism_tercil <- glm(tercil_superior_fundamentalism~X049+Gender+rend_subj+region+ethnic_group+
                                    idade_faixa+escolaridade_niveis+
                                    status_emplo+religion+autoloc_dir_esq+interesse,
                                  data = df, family=binomial(link=logit))

logitdesconfia_tercil <- glm(tercil_superior_desconfia~X049+Gender+rend_subj+region+ethnic_group+
                               idade_faixa+escolaridade_niveis+
                               status_emplo+religion+autoloc_dir_esq+interesse,
                             data = df, family=binomial(link=logit))

logitpromercado_tercil <- glm(tercil_superior_promercado~X049+Gender+rend_subj+region+ethnic_group+
                                idade_faixa+escolaridade_niveis+
                                status_emplo+religion+autoloc_dir_esq+interesse,
                              data = df, family=binomial(link=logit))

logitantidemocr_tercil <- glm(tercil_superior_antidemocr~X049+Gender+rend_subj+region+ethnic_group+
                                idade_faixa+escolaridade_niveis+
                                status_emplo+religion+autoloc_dir_esq+interesse,
                              data = df, family=binomial(link=logit))
...
library(lsr)
df$Unidad_Ciudadano <- as.numeric(df$Unidad_Ciudadano)
model<- aov(Unidad_Ciudadano ~ Gender+X049+ethnic_group+
              idade_faixa+escolaridade_niveis+
...
dfao <- read_sav("dfao.sav")
dfao <- dfao %>%
  filter(S002VS == 7 & S003==32)#argentina 
table(dfao$E179WVS)
...
df$votos_partidos <- ifelse(dfao$E179WVS < 5, "Nenhum/NSabe",
                            ifelse(dfao$E179WVS == 32001, "Union Ciudadana",
                                   ifelse(dfao$E179WVS == 32101, "FPV",
                                          ifelse(dfao$E179WVS == 32009, "Juntos por El Cambio",
                                                 ifelse(dfao$E179WVS == 32105, "PRO", "Outros Partidos")))))

# Converter a coluna para o tipo "factor" com a ordem correta dos níveis
df$votos_partidos <- factor(df$votos_partidos,
                            levels = c("Union Ciudadana", "FPV",
                                       "Juntos por El Cambio",
                                       "PRO", "Outros Partidos", "Nenhum/NSabe"))
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Argentina Onda 7 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
