...
df$X003R2[df$X003R2==-2] <- NA
df$X003R2 -> df$idade_faixa#idade_faixa
table(df$idade_faixa, useNA = "always")
...
df$status_emplo <- factor(ifelse(df$X028 == 1, "Full Time",
                          ...
                          levels = c("Full Time", "Housewife","Student", "Others"))
...
prop.table(table(df$X051))
df$color_race_ladino_guatemala <- df$X051 == 320001
table(df$color_race_ladino_guatemala)
...
dfao$X047R_WVS
table(df$X047R_WVS)
#tratar como levels dado o pequeno numero de High (juntar high e medium)
#High/Medium como referencia
df$rend_subj_level_recoded <- factor(df$X047R_WVS, levels = c(-1,1, 2, 3), labels = c("DK", "Low", "Medium/High", "Medium/High"))
df$rend_subj_level_recoded <- factor(df$rend_subj_level_recoded, levels = c("DK", "Low", "Medium/High"))
df$rend_subj_level_recoded <- relevel(df$rend_subj_level_recoded, ref = "Medium/High")
table(df$rend_subj_level_recoded)
...
df$GANA_ <- df$E179WVS == 320001
table(df$GANA_)
df$GANA_ <- as.numeric(df$GANA_)
df$GANA_ <- as.factor(df$GANA_)
table(df$GANA_)

# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 320001 , "GANA_",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("GANA_", "DK/NA/blank", "Others"))
...
prop.table(table(df$GANA_))
df <- subset(df, select=c(X049,Gender,idade_faixa,color_race_ladino_guatemala,rend_subj_level_recoded,
                          escolaridade_niveis,status_emplo,religion,
                          autoloc_dir_esq, interesse,
                          GANA_,df_voto_multinom,
                          E069_07,E069_12,E069_17,E069_06,F028,
                          E037,E040,E036,E116,E114,E117,F120,F118,E039,E235))
summary(df[,1:12])
df[,13:27]#issues - ok, consta todos?

df <- subset(df, select=-c(E069_17,E040,E235))

summary(df[,13:24])#ver se tem que remover algo
...
print(df[, 13:24])
summary(df[,13:24])
imp <- mice(df[,13:24], seed=23109)# o nome da base e a seed sempre essa 23109
df[,13:24] <- complete(imp, 1)#sempre a m como  destino da mputação , empre escolher a 1
rm(imp)
summary(df[,13:24])
...
df <- subset(df, select=c(X049,Gender,idade_faixa,color_race_ladino_guatemala,rend_subj_level_recoded,
                          ...
                          autoloc_dir_esq, interesse,
                          GANA_,df_voto_multinom,
                          E069_07,E069_12,E069_06,F118_invertido,
                          F120_invertido,F028_invertido,
                          E037,E036_invertido,
                          E116_invertido,E114_invertido,E117,E039_invertido))#nao tem mais E069_17,E040,E235
...
model <- '
  ...
  Desconfiança =~ E069_07 + E069_12 + E069_06
  AntiDemocracia =~ E117 + E114_invertido + E116_invertido
  ProMercado_Meritocracia =~E039_invertido+E037 + E036_invertido 
...
#scores[,4] -> df$promercado # abdiquei
...
#modelos
logitGANA__1 <- glm(GANA_~X049+Gender+color_race_ladino_guatemala+rend_subj_level_recoded+
                    idade_faixa+escolaridade_niveis+
                    status_emplo+religion,data = df, family=binomial(link=logit))
logitGANA__2 <- glm(GANA_~fundamentalism+antidemocr+
                    desconfia,data = df, family=binomial(link=logit))
logitGANA__3 <- glm(GANA_~autoloc_dir_esq+interesse+
                    fundamentalism+antidemocr+
                    desconfia,data = df, family=binomial(link=logit))
logitGANA__4 <- glm(GANA_~X049+Gender+color_race_ladino_guatemala+rend_subj_level_recoded+
                    idade_faixa+escolaridade_niveis+
                    status_emplo+religion+autoloc_dir_esq+interesse+
                    fundamentalism+antidemocr+
                    desconfia,
                  data = df, family=binomial(link=logit))
tab_model(logitGANA__1,logitGANA__2,logitGANA__3,logitGANA__4,
...
bic_modelo1 <- BIC(logitGANA__1)
bic_modelo2 <- BIC(logitGANA__2)
bic_modelo3 <- BIC(logitGANA__3)
bic_modelo4 <- BIC(logitGANA__4)
...
# Definir "GANA_" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "GANA_")

library(nnet)
modeloMulti <- multinom(df_voto_multinom ~ X049+Gender+color_race_ladino_guatemala+rend_subj_level_recoded+
                          idade_faixa+escolaridade_niveis+
                          status_emplo+religion+autoloc_dir_esq+interesse+
                          fundamentalism+antidemocr+
                          desconfia,
...
logitfundamentalism_tercil <- glm(tercil_superior_fundamentalism~X049+Gender+
                                    color_race_ladino_guatemala+rend_subj_level_recoded+
                                    idade_faixa+escolaridade_niveis+
                                    status_emplo+religion+autoloc_dir_esq+interesse,
                                  data = df, family=binomial(link=logit))
...
logitantidemocr_tercil <- glm(tercil_superior_antidemocr~X049+Gender+color_race_ladino_guatemala+rend_subj_level_recoded+
                                idade_faixa+escolaridade_niveis+
                                status_emplo+religion+autoloc_dir_esq+interesse,
                              data = df, family=binomial(link=logit))
...
library(lsr)
df$GANA_ <- as.numeric(df$GANA_)
model<- aov(GANA_ ~ Gender+X049+color_race_ladino_guatemala+
              idade_faixa+escolaridade_niveis+rend_subj_level_recoded+
...
df$votos_partidos <- ifelse(dfao$E179WVS < 5, "Nenhum/NSabe",
                            ifelse(dfao$E179WVS == 320001, "UNE",
                                   ifelse(dfao$E179WVS == 320002, "GANA",
                                          ifelse(dfao$E179WVS == 320004, "Frente Republicano",
                                                 ifelse(dfao$E179WVS == 320003, "Avanzada Nacional",
                                                        "Outros Partidos")))))

# Definir a ordem dos níveis na coluna "votos_partidos"
df$votos_partidos <- factor(df$votos_partidos, levels = c("UNE", "GANA", "Frente Republicano",
                                                          "Avanzada Nacional", "Outros Partidos", "Nenhum/NSabe"))
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Guatemala Onda 5",y="Probabilidade") + theme_minimal()
fundamentalismo
