...
df <- read_csv("df.csv")
df <- df %>%
  filter(S002VS == 3 & S003==858)#urug3
...
df$FrenteAmplio_ <- df$E179WVS == 858003 
table(df$FrenteAmplio_)
df$FrenteAmplio_ <- as.numeric(df$FrenteAmplio_)
df$FrenteAmplio_ <- as.factor(df$FrenteAmplio_)
table(df$FrenteAmplio_)
prop.table(table(df$FrenteAmplio_))
# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 858003 , "FrenteAmplio_",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3,4),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("FrenteAmplio_", "DK/NA/blank", "Others"))
...
df <- subset(df, select=c(raca_branc,Gender,idade_faixa,status_emplo,rend_subj,religion,X049,
                          autoloc_dir_esq, interesse,escolaridade_niveis,region,
                          FrenteAmplio_,df_voto_multinom,
                          E069_07,E069_12,E069_17,E069_06,F028,
                          E037,E040,E036,E116,E114,E117,F120,F118,E039,E235))
summary(df)
summary(df[,1:13])
df[,14:28]#issues, remover E235
df <- subset(df, select = -c(E235))#remover colunnas de issues nao usadas na confirmatóriaa
...
#remover E116,E114,E117,E037,E235
df <- subset(df, select=c(raca_branc,Gender,idade_faixa,status_emplo,rend_subj,religion,X049,
                          autoloc_dir_esq, interesse,escolaridade_niveis,region,
                          FrenteAmplio_,df_voto_multinom,
...
df <- subset(df, select=c(raca_branc,Gender,idade_faixa,status_emplo,rend_subj,religion,region,
                          autoloc_dir_esq, interesse,escolaridade_niveis,X049,
                          FrenteAmplio_,df_voto_multinom,
                          fundamentalism,desconfia,promercado,antidemocr))
rm(dfao,fit)
summary(df)
#modelos
logitFrenteAmplio__1 <- glm(FrenteAmplio_~Gender+region+X049+raca_branc+
                          idade_faixa+
                          status_emplo+rend_subj+religion,data = df, family=binomial(link=logit))
logitFrenteAmplio__2 <- glm(FrenteAmplio_~fundamentalism+antidemocr+
                          desconfia+promercado,data = df, family=binomial(link=logit))
logitFrenteAmplio__3 <- glm(FrenteAmplio_~autoloc_dir_esq+interesse+escolaridade_niveis+
                          fundamentalism+antidemocr+
                          desconfia+promercado,data = df, family=binomial(link=logit))
logitFrenteAmplio__4 <- glm(FrenteAmplio_~Gender+region+X049+raca_branc+
                          idade_faixa+
                          status_emplo+rend_subj+religion+autoloc_dir_esq+interesse+escolaridade_niveis+
                          fundamentalism+antidemocr+
                          desconfia+promercado,
                        data = df, family=binomial(link=logit))
tab_model(logitFrenteAmplio__1,logitFrenteAmplio__2,logitFrenteAmplio__3,logitFrenteAmplio__4,
...
# Definir "FrenteAmplio_" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "FrenteAmplio_")

library(nnet)
modeloMulti <- multinom(df_voto_multinom ~ Gender+region+X049+raca_branc+
                          idade_faixa+
                          status_emplo+rend_subj+religion+autoloc_dir_esq+interesse+escolaridade_niveis+
                          fundamentalism+antidemocr+
                          desconfia+promercado,
...
logitfundamentalism_tercil <- glm(tercil_superior_fundamentalism~Gender+region+X049+
                                    idade_faixa+escolaridade_niveis+
                                    status_emplo+rend_subj+religion+autoloc_dir_esq+interesse,
                                  data = df, family=binomial(link=logit))

logitdesconfia_tercil <- glm(tercil_superior_desconfia~Gender+region+X049+
                               idade_faixa+escolaridade_niveis+
                               status_emplo+rend_subj+religion+autoloc_dir_esq+interesse,
                             data = df, family=binomial(link=logit))

logitantidemocr_tercil <- glm(tercil_superior_antidemocr~Gender+region+X049+
                                idade_faixa+escolaridade_niveis+
                                status_emplo+rend_subj+religion+autoloc_dir_esq+interesse,
                              data = df, family=binomial(link=logit))

logitpromercado_tercil <- glm(tercil_superior_promercado~Gender+region+X049+
                                idade_faixa+escolaridade_niveis+
                                status_emplo+rend_subj+religion+autoloc_dir_esq+interesse,
                              data = df, family=binomial(link=logit))
...
library(lsr)
df$FrenteAmplio_ <- as.numeric(df$FrenteAmplio_)
model<- aov(FrenteAmplio_ ~ Gender+X049+raca_branc+
              idade_faixa+escolaridade_niveis+
...
dfao <- read_sav("dfao.sav")
dfao <- dfao %>%
  filter(S002VS == 3 & S003==858)
table(dfao$E179WVS)
#
# Criar a nova coluna "votos_partidos" no dataframe "df"
df$votos_partidos <- ifelse(dfao$E179WVS < 5, "Nenhum/NSabe",
                            ifelse(dfao$E179WVS == 858003, "Frente Amplio",
                                   ifelse(dfao$E179WVS == 858005, "Nuevo Espacio",
                                          ifelse(dfao$E179WVS == 858001, "Colorado",
                                                 ifelse(dfao$E179WVS == 858002, "Nacional",
                                                        "Outros Partidos")))))

# Definir a ordem dos níveis na coluna "votos_partidos"
df$votos_partidos <- factor(df$votos_partidos, levels = c("Frente Amplio", "Nuevo Espacio", "Colorado", "Nacional", "Outros Partidos", "Nenhum/NSabe"))
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Uruguai Onda 3 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
