...
df$escolaridade_niveis[df$escolaridade_niveis==-2]<-NA
...
df$status_emplo <- factor(ifelse(df$X028 == 1, "Full Time",
                                 ifelse(df$X028 == 3, "Self Employed",
                                        ifelse(df$X028 == 5, "Housewife", "Others"))),
                          levels = c("Full Time", "Self Employed", "Housewife", "Others"))
...
dfao$X047R_WVS
table(df$X047R_WVS)
df$rend_subj <- factor(df$X047R_WVS, levels = c(1, 2, 3), labels = c("Low", "Medium", "High"))
df$rend_subj <- factor(df$rend_subj, levels = c("Low", "Medium", "High"))
...
droplevels(df$autoloc_dir_esq)-> df$autoloc_dir_esq
table(df$autoloc_dir_esq)

#dep
table(df$E179WVS)

df$PSUV <- df$E179WVS == 862031
table(df$PSUV)
df$PSUV <- as.numeric(df$PSUV)
df$PSUV <- as.factor(df$PSUV)
table(df$PSUV)
prop.table(table(df$PSUV))
# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 862031 , "PSUV",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3,4),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("PSUV", "DK/NA/blank", "Others"))
...
model <- '
  ...
  AntiDemocracia =~ E117  + E116_invertido+E235_invertido
  ...
  E117 ~~ E117
'
dfao$E037#pro estado # removi e E114 também
...
df <- subset(df, select=c(raca_branc,Gender,idade_faixa,status_emplo,rend_subj,religion,X049,region,
...
summary(df)
#modelos
logitPSUV_1 <- glm(PSUV~Gender+region+raca_branc+X049+
                              idade_faixa+
                              status_emplo+rend_subj+religion,data = df, family=binomial(link=logit))
logitPSUV_2 <- glm(PSUV~fundamentalism+antidemocr+
                              desconfia+promercado,data = df, family=binomial(link=logit))
logitPSUV_3 <- glm(PSUV~autoloc_dir_esq+interesse+escolaridade_niveis+
                              fundamentalism+antidemocr+
                              desconfia+promercado,data = df, family=binomial(link=logit))
logitPSUV_4 <- glm(PSUV~Gender+region+raca_branc+X049+
                              idade_faixa+
                              status_emplo+rend_subj+religion+autoloc_dir_esq+interesse+escolaridade_niveis+
                              fundamentalism+antidemocr+
                              desconfia+promercado,
                            data = df, family=binomial(link=logit))
tab_model(logitPSUV_1,logitPSUV_2,logitPSUV_3,logitPSUV_4,
...
# Definir "PSUV" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "PSUV")

library(nnet)
modeloMulti <- multinom(df_voto_multinom ~ Gender+region+raca_branc+X049+
                          idade_faixa+
                          status_emplo+rend_subj+religion+autoloc_dir_esq+interesse+escolaridade_niveis+
                          fundamentalism+antidemocr+
                          desconfia+promercado,
...
library(lsr)
df$PSUV <- as.numeric(df$PSUV)
model<- aov(PSUV ~ Gender+X049+raca_branc+
              idade_faixa+escolaridade_niveis+
...
df$votos_partidos <- ifelse(dfao$E179WVS < 5, "Nenhum/NSabe",
                            ifelse(dfao$E179WVS == 862031, "PSUV",
                                   ifelse(dfao$E179WVS == 862006, "Primero Justicia",
                                          ifelse(dfao$E179WVS == 862032, "Voluntad Popular",
                                                 ifelse(dfao$E179WVS == 862002, "Acción Democrática",
                                                        "Outros Partidos")))))

# Definir a ordem dos níveis na coluna "votos_partidos"
df$votos_partidos <- factor(df$votos_partidos, levels = c("PSUV", "Primero Justicia", "Voluntad Popular", "Acción Democrática", "Outros Partidos", "Nenhum/NSabe"))
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Venezuela Onda 7 WVS WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
