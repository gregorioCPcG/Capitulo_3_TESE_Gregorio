...
df <- read_csv("df.csv")
df <- df %>%
  filter(S002VS == 5 & S003==604)#peru5
...
df$rend_subj <- relevel(df$rend_subj, ref = "Low")
...
df$APRA <- df$E179WVS == 604002 
table(df$APRA)
df$APRA <- as.numeric(df$APRA)
df$APRA <- as.factor(df$APRA)
table(df$APRA)
prop.table(table(df$APRA))
# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 604002 , "APRA",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3,4),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("APRA", "DK/NA/blank", "Others"))
...
df <- subset(df, select=c(raca_branc,Gender,idade_faixa,status_emplo,rend_subj,religion,
                          ...
                          APRA,df_voto_multinom,
                          E069_07,E069_12,E069_17,E069_06,F028,
                          E037,E040,E036,E116,E114,E117,F120,F118,E039,E235))
summary(df)
summary(df[,1:11])
df[,12:26]#issues, remover F120 e F118
df <- subset(df, select = -c(F120,F118))#remover colunnas de issues nao usadas na confirmatóriaa
...
print(df[, 12:24])
summary(df[,12:24])
imp <- mice(df[,12:24], seed=23109)# o nome da base e a seed sempre essa 23109
df[,12:24] <- complete(imp, 1)#sempre a m como  destino da mputação , empre escolher a 1
rm(imp)
summary(df[,12:24])
...
df <- subset(df, select=c(raca_branc,Gender,idade_faixa,status_emplo,rend_subj,religion,
                          autoloc_dir_esq, interesse,escolaridade_niveis,
                          APRA,df_voto_multinom,
                          E069_07,E069_17,E069_06,F028_invertido,
...
psych::scree(df[,12:23])
nfactors((df[,12:23]))
# Especifique o modelo de CFA com 3 fatores
#Por ausencia de questões F118 e F120 o liberal/fundamentalismo não foi rodado
model <- '
...
#scores[,1] -> df$fundamentalism#removido por ausencia de questões
scores[,1] -> df$desconfia#1
#dfao$E117#ok
scores[,2] -> df$antidemocr#2
scores[,3] -> df$promercado #3
...
df <- subset(df, select=c(raca_branc,Gender,idade_faixa,status_emplo,rend_subj,religion,
                          autoloc_dir_esq, interesse,escolaridade_niveis,
                          APRA,df_voto_multinom,
                          desconfia,promercado,antidemocr))
rm(dfao,fit)
summary(df)
#modelos
logitAPRA_1 <- glm(APRA~Gender+raca_branc+
                          idade_faixa+
                          status_emplo+rend_subj+religion,data = df, family=binomial(link=logit))
logitAPRA_2 <- glm(APRA~antidemocr+
                          desconfia+promercado,data = df, family=binomial(link=logit))
logitAPRA_3 <- glm(APRA~autoloc_dir_esq+interesse+escolaridade_niveis+
                          antidemocr+
                          desconfia+promercado,data = df, family=binomial(link=logit))
logitAPRA_4 <- glm(APRA~Gender+raca_branc+
                          idade_faixa+
                          status_emplo+rend_subj+religion+autoloc_dir_esq+interesse+escolaridade_niveis+
                          antidemocr+
                          desconfia+promercado,
                        data = df, family=binomial(link=logit))
tab_model(logitAPRA_1,logitAPRA_2,logitAPRA_3,logitAPRA_4,
...
# Definir "APRA" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "APRA")
...
# OBS para mais testes ver molde-logitfundamentalism_tercil
tab_model(logitdesconfia_tercil,logitpromercado_tercil,
...
library(lsr)
df$APRA <- as.numeric(df$APRA)
model<- aov(APRA ~ Gender++raca_branc+
              idade_faixa+escolaridade_niveis+
...
dfao <- read_sav("dfao.sav")
dfao <- dfao %>%
  filter(S002VS == 5 & S003==604)
#
table(dfao$E179WVS)
#
# Criar a nova coluna "votos_partidos" no dataframe "df"
df$votos_partidos <- ifelse(dfao$E179WVS < 5, "Nenhum/NSabe",
                            ifelse(dfao$E179WVS == 604023, "Nacionalista Peruano",
                                   ifelse(dfao$E179WVS == 604002, "APRA",
                                          ifelse(dfao$E179WVS == 604004, "Unidad Nacional",
                                                 ifelse(dfao$E179WVS == 604021, "Restauración Nacional",
                                                        "Outros Partidos")))))

# Definir a ordem dos níveis na coluna "votos_partidos"
df$votos_partidos <- factor(df$votos_partidos, levels = c("Nacionalista Peruano", "APRA", "Unidad Nacional", "Restauración Nacional", "Outros Partidos", "Nenhum/NSabe"))
...
antidemocr<-antidemocr+labs(title="Prob. tercil superior de Antidemocracia",
                            x= "Partidos Peru Onda 5 WVS",y="Probabilidade") + theme_minimal()
antidemocr
...
grid.arrange(desconfia,promercado,antidemocr,nrow=2)
