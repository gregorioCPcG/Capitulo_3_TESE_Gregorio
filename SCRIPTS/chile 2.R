...
df <- read_csv("df.csv")
df <- df %>%
  filter(S002VS == 2 & S003==152)#chile2
...
df <- subset(df, select=c(X049,raca_branc,Gender,idade_faixa,
                          region,status_emplo,rend_subj,religion,
