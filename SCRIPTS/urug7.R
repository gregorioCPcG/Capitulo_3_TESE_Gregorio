...
df <- read_csv("df.csv")
df <- df %>%
  filter(S002VS == 7 & S003==858)#urug7
...
summary(df)
#modelos
logitFrenteAmplio__1 <- glm(FrenteAmplio_~Gender+region+raca_branc+X049+
                              idade_faixa+
...
logitFrenteAmplio__3 <- glm(FrenteAmplio_~autoloc_dir_esq+interesse+escolaridade_niveis+
                              ...
                              desconfia+promercado,data = df, family=binomial(link=logit))
logitFrenteAmplio__4 <- glm(FrenteAmplio_~Gender+region+raca_branc+X049+
                              idade_faixa+
...
df$votos_partidos <- factor(df$votos_partidos, levels = c("Frente Amplio",
                                                          "Colorado", "Nacional", "Outros Partidos",
                                                          "Nenhum/NSabe"))
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Uruguai Onda 7 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
