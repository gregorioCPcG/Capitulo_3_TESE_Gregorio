...
df$RN_ <- df$E179WVS == 152002
table(df$RN_)
df$RN_ <- as.numeric(df$RN_)
df$RN_ <- as.factor(df$RN_)
table(df$RN_)

# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 152002 , "RN_",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("RN_", "DK/NA/blank", "Others"))
...
bic_modelo1 <- BIC(logitRN__1)
bic_modelo2 <- BIC(logitRN__2)
bic_modelo3 <- BIC(logitRN__3)
bic_modelo4 <- BIC(logitRN__4)
...
# Definir "RN_" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "RN_")
...
library(lsr)
df$RN_ <- as.numeric(df$RN_)
model<- aov(RN_ ~ Gender+X049+raca_branc+
              idade_faixa+escolaridade_niveis+
...
df$votos_partidos <- ifelse(dfao$E179WVS < 5, "Nenhum/NSabe",
                            ifelse(dfao$E179WVS == 152009, "Comunista",
...
df$votos_partidos <- factor(df$votos_partidos, levels = c("Comunista", "Socialista", "PPD",
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Chile Onda 7 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
