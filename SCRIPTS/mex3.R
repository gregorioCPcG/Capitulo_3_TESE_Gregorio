...
#imputar como média (-1)
df$X049[df$X049==-1]<- NA

sum(is.na(df$X049))
mean(df$X049, na.rm = T)
dfao$X049 -> df$X049
df$X049[df$X049==-1]<- 5.39
table(df$X049)
...
df$PRI_ <- df$E179WVS == 484001
table(df$PRI_)
df$PRI_ <- as.numeric(df$PRI_)
df$PRI_ <- as.factor(df$PRI_)
table(df$PRI_)
prop.table(table(df$PRI_))
# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 484001 , "PRI_",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3,4),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("PRI_", "DK/NA/blank", "Others"))
...
print(df[, 14:26])
summary(df[,14:26])
imp <- mice(df[,14:26], seed=23109)# o nome da base e a seed sempre essa 23109
df[,14:26] <- complete(imp, 1)#sempre a m como  destino da mputação , empre escolher a 1
rm(imp)
summary(df[,14:26])
...
bic_modelo1 <- BIC(logitPRI__1)
bic_modelo2 <- BIC(logitPRI__2)
bic_modelo3 <- BIC(logitPRI__3)
bic_modelo4 <- BIC(logitPRI__4)
...
# Definir "PRI_" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "PRI_")
...
logitpromercado_tercil <- glm(tercil_superior_promercado~Gender+X049+
                                idade_faixa+escolaridade_niveis+
                                status_emplo+rend_subj+religion+autoloc_dir_esq+interesse,
                              data = df, family=binomial(link=logit))
...
library(lsr)
df$PRI_ <- as.numeric(df$PRI_)
model<- aov(PRI_ ~ Gender+X049+raca_branc+
              idade_faixa+escolaridade_niveis+
...
df$votos_partidos <- factor(df$votos_partidos, levels = c("PT", "PRD", "PRI", "PAN", "Outros Partidos", "Nenhum/NSabe"))
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos México Onda 3 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
