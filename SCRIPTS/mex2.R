...
#print(dfao$X025R)
table(df$X025R, useNA = "always")#nao tem escolaridade
...
df <- subset(df, select=c(X049,raca_branc,Gender,idade_faixa,status_emplo,rend_subj,religion,
...
df <- subset(df, select = -c(E235,E117,E114,E116,
...
print(df[, 12:21])
summary(df[,12:21])
imp <- mice(df[,12:21], seed=23109)# o nome da base e a seed sempre essa 23109
df[,12:21] <- complete(imp, 1)#sempre a m como  destino da mputação , empre escolher a 1
rm(imp)
summary(df[,12:21])
...
psych::scree(df[,12:21])
nfactors((df[,12:21]))
# Especifique o modelo de CFA com 4 fatores / remover E116,E114,E117,E037,E235
model <- '
  # Especificação do modelo fatorial
  LiberalFundamentalismo =~ F118_invertido + F120_invertido + F028_invertido
  Desconfiança =~ E069_07 + E069_17 + E069_06 + E069_12
