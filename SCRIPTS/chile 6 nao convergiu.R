...
df$PartidoSocialista_onda6 <- df$E179WVS == 152007
table(df$PartidoSocialista_onda6)
prop.table((table(df$PartidoSocialista_onda6)))
df$PartidoSocialista_onda6 <- as.numeric(df$PartidoSocialista_onda6)
df$PartidoSocialista_onda6 <- as.factor(df$PartidoSocialista_onda6)
table(df$PartidoSocialista_onda6)

# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 152007 , "PartidoSocialista_onda6",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("PartidoSocialista_onda6", "DK/NA/blank", "Others"))
...
df <- subset(df, select=c(raca_branc,Gender,idade_faixa,X049,
                          ...
                          PartidoSocialista_onda6,df_voto_multinom,
...
fit <- cfa(model, data = df,model.type="cfa")
