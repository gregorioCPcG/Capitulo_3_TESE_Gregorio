...
df <- read_csv("df.csv")
df <- df %>%
  filter(S002VS == 6 & S003==76)#bra_onda6
...
df$X049[df$X049==-2]<- NA
df$X049[df$X049==-1]<- NA
sum(is.na(df$X049))#nao vou abrir mao de 180 casos, imputarei a média
mean_X049 <- mean(df$X049, na.rm = TRUE)
df$X049[is.na(df$X049)] <- mean_X049
rm(mean_X049)
sum(is.na(df$X049))
summary(df$X049)
...
table(df$renda_subj_numeric, useNA = "always" )#ok 27 NAS
