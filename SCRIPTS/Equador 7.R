...
df$escolaridade_niveis[df$escolaridade_niveis==-5]<-NA
...
dfao$X047R_WVS
table(df$X047R_WVS)
df$rend_subj_numericc <- as.numeric(df$X047R_WVS)
df$rend_subj_numericc[df$rend_subj_numericc<0]<-NA

table(df$rend_subj_numericc,useNA = "always")
...
df$Alianza_PAIS <- df$E179WVS == 218001
table(df$Alianza_PAIS)
df$Alianza_PAIS <- as.numeric(df$Alianza_PAIS)
df$Alianza_PAIS <- as.factor(df$Alianza_PAIS)
table(df$Alianza_PAIS)
prop.table(table(df$Alianza_PAIS))
# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 218001 , "Alianza_PAIS",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("Alianza_PAIS", "DK/NA/blank", "Others"))
...
df <- subset(df, select=c(raca_branc,Gender,idade_faixa,X049,
                          escolaridade_niveis,status_emplo,rend_subj_numericc,religion,
                          autoloc_dir_esq, interesse,
                          Alianza_PAIS,df_voto_multinom,
...
df <- subset(df, select=c(raca_branc,Gender,idade_faixa,X049,
                          escolaridade_niveis,status_emplo,rend_subj_numericc,religion,
                          autoloc_dir_esq, interesse,Alianza_PAIS,df_voto_multinom,
                          fundamentalism,desconfia,antidemocr,promercado))
rm(dfao,fit)
summary(df)
#modelos
logitAlianza_PAIS_1 <- glm(Alianza_PAIS~Gender+X049+
                    idade_faixa+escolaridade_niveis+
                    status_emplo+rend_subj_numericc+religion,data = df, family=binomial(link=logit))
logitAlianza_PAIS_2 <- glm(Alianza_PAIS~fundamentalism+antidemocr+promercado+
                    desconfia,data = df, family=binomial(link=logit))
logitAlianza_PAIS_3 <- glm(Alianza_PAIS~autoloc_dir_esq+interesse+
                    fundamentalism+antidemocr+promercado+
                    desconfia,data = df, family=binomial(link=logit))
logitAlianza_PAIS_4 <- glm(Alianza_PAIS~Gender+X049+
                    idade_faixa+escolaridade_niveis+
                    status_emplo+rend_subj_numericc+religion+autoloc_dir_esq+interesse+
                    fundamentalism+antidemocr+promercado+
                    desconfia,
                  data = df, family=binomial(link=logit))
tab_model(logitAlianza_PAIS_1,logitAlianza_PAIS_2,logitAlianza_PAIS_3,logitAlianza_PAIS_4,
...
bic_modelo1 <- BIC(logitAlianza_PAIS_1)
bic_modelo2 <- BIC(logitAlianza_PAIS_2)
bic_modelo3 <- BIC(logitAlianza_PAIS_3)
bic_modelo4 <- BIC(logitAlianza_PAIS_4)
...
# Definir "Alianza_PAIS" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "Alianza_PAIS")

library(nnet)
modeloMulti <- multinom(df_voto_multinom ~ Gender+X049+
                          idade_faixa+escolaridade_niveis+
                          status_emplo+rend_subj_numericc+religion+autoloc_dir_esq+interesse+
                          fundamentalism+antidemocr+promercado+
                          desconfia,
...
logitfundamentalism_tercil <- glm(tercil_superior_fundamentalism~Gender+X049+
                                    idade_faixa+escolaridade_niveis+raca_branc+
                                    status_emplo+rend_subj_numericc+religion+autoloc_dir_esq+interesse,
                                  data = df, family=binomial(link=logit))
...
logitpromercado_tercil <- glm(tercil_superior_promercado~X049+Gender+raca_branc+
                                idade_faixa+escolaridade_niveis+rend_subj_numericc+
...
library(lsr)
df$Alianza_PAIS <- as.numeric(df$Alianza_PAIS)
model<- aov(Alianza_PAIS ~ Gender+X049+raca_branc+
              idade_faixa+escolaridade_niveis+
...
df$votos_partidos <- ifelse(dfao$E179WVS < 5, "Nenhum/NSabe",
                            ifelse(dfao$E179WVS == 218001, "Alianza PAIS",
                                   ifelse(dfao$E179WVS == 218014, "CREO",
                                          "Outros Partidos")))

# Definir a ordem dos níveis na coluna "votos_partidos"
df$votos_partidos <- factor(df$votos_partidos, levels = c("Alianza PAIS", "CREO", "Outros Partidos", "Nenhum/NSabe"))
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Equador Onda 7 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
