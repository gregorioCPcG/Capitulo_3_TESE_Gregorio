...
df <- read_csv("df.csv")
df <- df %>%
  filter(S002VS == 3 & S003==32)#argentina 3
...
print(dfao$X049)
table(df$X049)#cidade tamanho # NAO TEM
...
df <- subset(df, select=c(region,Gender,idade_faixa,
...
summary(df[,1:11])
df[,12:26]#issues
summary(df[,12:26])#ver se tem que remover algo
df <- subset(df, select=-c(E235))
df[,12:25]
summary(df[,12:25])#ver se tem que remover algo
...
#modelos
logitJusticialista_1 <- glm(Justicialista~region+Gender+
...
logitJusticialista_3 <- glm(Justicialista~autoloc_dir_esq+interesse+
                                ...
                                desconfia+promercado,data = df, family=binomial(link=logit))
logitJusticialista_4 <- glm(Justicialista~region+Gender+
                                idade_faixa+escolaridade_niveis+
...
library(nnet)
modeloMulti <- multinom(df_voto_multinom ~ region+Gender+
                           idade_faixa+escolaridade_niveis+
                           status_emplo+rend_subj+religion+autoloc_dir_esq+interesse+
                           fundamentalism+antidemocr+
                           desconfia+promercado,
...
logitfundamentalism_tercil <- glm(tercil_superior_fundamentalism~region+Gender+
                                    idade_faixa+escolaridade_niveis+
                                    status_emplo+rend_subj+religion+autoloc_dir_esq+interesse,
                                  data = df, family=binomial(link=logit))

logitdesconfia_tercil <- glm(tercil_superior_desconfia~region+Gender+
                                    idade_faixa+escolaridade_niveis+
                                    status_emplo+rend_subj+religion+autoloc_dir_esq+interesse,
                                  data = df, family=binomial(link=logit))

logitpromercado_tercil <- glm(tercil_superior_promercado~region+Gender+
                                    idade_faixa+escolaridade_niveis+
                                    status_emplo+rend_subj+religion+autoloc_dir_esq+interesse,
                                  data = df, family=binomial(link=logit))

logitantidemocr_tercil <- glm(tercil_superior_antidemocr~region+Gender+
                                    idade_faixa+escolaridade_niveis+
                                    status_emplo+rend_subj+religion+autoloc_dir_esq+interesse,
                                  data = df, family=binomial(link=logit))
...
library(lsr)
df$Justicialista <- as.numeric(df$Justicialista)
model<- aov(Justicialista ~ Gender+
              idade_faixa+escolaridade_niveis+
              status_emplo+rend_subj+religion, data= df)
etaSquared(model)
summary(model)# se quiser 
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                          x= "Partidos Argentina Onda 3 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
