...
df$LiberalParty <- df$E179WVS == 170002
table(df$LiberalParty)
df$LiberalParty <- as.numeric(df$LiberalParty)
df$LiberalParty <- as.factor(df$LiberalParty)
table(df$LiberalParty)

# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 170002 , "LiberalParty",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("LiberalParty", "DK/NA/blank", "Others"))
...
prop.table(table(df$LiberalParty))
...
#dfao$E117#ok
#scores[,3] -> df$antidemocr abdicar
scores[,4] -> df$promercado # nao usamos
rm(scores,labels,model,values)

df <- subset(df, select=c(raca_branc,Gender,idade_faixa,X049,
                          escolaridade_niveis,status_emplo,rend_subj,religion,
                          autoloc_dir_esq, interesse,LiberalParty,df_voto_multinom,
                          fundamentalism,desconfia,promercado,region))
rm(dfao,fit)
summary(df)
#modelos
logitLiberalParty_1 <- glm(LiberalParty~Gender+X049+region+
                    idade_faixa+escolaridade_niveis+
                    status_emplo+rend_subj+religion,data = df, family=binomial(link=logit))
logitLiberalParty_2 <- glm(LiberalParty~fundamentalism+promercado+
                    desconfia,data = df, family=binomial(link=logit))
logitLiberalParty_3 <- glm(LiberalParty~autoloc_dir_esq+interesse+
                    fundamentalism+promercado+
                    desconfia,data = df, family=binomial(link=logit))
logitLiberalParty_4 <- glm(LiberalParty~Gender+X049+region+
                    idade_faixa+escolaridade_niveis+
                    status_emplo+rend_subj+religion+autoloc_dir_esq+interesse+
                    fundamentalism+promercado+
                    desconfia,
                  data = df, family=binomial(link=logit))
tab_model(logitLiberalParty_1,logitLiberalParty_2,logitLiberalParty_3,logitLiberalParty_4,
...
bic_modelo1 <- BIC(logitLiberalParty_1)
bic_modelo2 <- BIC(logitLiberalParty_2)
bic_modelo3 <- BIC(logitLiberalParty_3)
bic_modelo4 <- BIC(logitLiberalParty_4)
...
# Definir "LiberalParty" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "LiberalParty")
...
logitpromercado_tercil <- glm(tercil_superior_promercado~X049+Gender+raca_branc+region+
...
library(lsr)
df$LiberalParty <- as.numeric(df$LiberalParty)
model<- aov(LiberalParty ~ Gender+X049+raca_branc+
              idade_faixa+escolaridade_niveis+
...
df$votos_partidos <- ifelse(dfao$E179WVS < 5, "Nenhum/NSabe",
                            ifelse(dfao$E179WVS == 170006, "Alianza Verde",
                                   ifelse(dfao$E179WVS == 170020, "PDA",
                                                 ifelse(dfao$E179WVS %in% c(170002), "Liberal",
                                                        ifelse(dfao$E179WVS == 170004, "Cambio Radical",
                                                               ifelse(dfao$E179WVS == 170001, "Conservador",
                                                                      ifelse(dfao$E179WVS == 170003, "LA U",
                                                                             ifelse(dfao$E179WVS == 170021, "Centro Democrático",
                                                                                    "Outros"))))))))
                            
# Definir a ordem dos níveis na coluna "votos_partidos"
df$votos_partidos <- factor(df$votos_partidos, levels = c("Alianza Verde", "PDA",
                                                          "Liberal", "Cambio Radical", "Conservador",
                                                          "LA U", "Centro Democrático",
                                                          "Outros", "Nenhum/NSabe"))
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Colômbia Onda 7 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo

desconfia<-plot_cap(logitdesconfia_tercil2, condition="votos_partidos",conf_level = .9)
desconfia<-desconfia+labs(title="Prob. tercil superior de Antidemocracia",
                            x= "Partidos Colômbia Onda 7 WVS",y="Probabilidade") + theme_minimal()
