...
print(df$X051)
table(df$X051)#variavel q nao tem na Bolívia, nao tem iomportancia
# Criando a coluna df$not_pertain_to_indigenous_group com base na coluna df$X051
#apenas binrizar
df$not_pertain_to_indigenous_group <- df$X051 == 68001


# Visualizando o resultado
table(df$not_pertain_to_indigenous_group)
...
dfao$X047R_WVS
table(df$X047R_WVS)
df$rend_subj <- factor(df$X047R_WVS, levels = c(-1,-2, 1, 2, 3),
...
df$MAS_ <- df$E179WVS == 68001
table(df$MAS_)
df$MAS_ <- as.numeric(df$MAS_)
df$MAS_ <- as.factor(df$MAS_)
table(df$MAS_)

# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 68001 , "MAS_",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("MAS_", "DK/NA/blank", "Others"))
...
df <- subset(df, select=c(X049,Gender,idade_faixa,rend_subj,region,not_pertain_to_indigenous_group,
                          escolaridade_niveis,status_emplo,religion,
                          autoloc_dir_esq, interesse,
                          MAS_,df_voto_multinom,
                          E069_07,E069_12,E069_17,E069_06,F028,
                          E037,E040,E036,E116,E114,E117,F120,F118,E039,E235))
summary(df[,1:13])
df[,14:28]#issues
#summary(df[,14:28])#ver se tem que remover algo
...
df <- subset(df, select=c(X049,Gender,idade_faixa,rend_subj,region,not_pertain_to_indigenous_group,
                          ...
                          fundamentalism,
                          desconfia,
                          antidemocr,promercado))
rm(dfao,fit)
summary(df)
#modelos
logitMAS__1 <- glm(MAS_~X049+Gender+rend_subj+region+not_pertain_to_indigenous_group+
                                 idade_faixa+escolaridade_niveis+
                                 status_emplo+religion,data = df, family=binomial(link=logit))
logitMAS__2 <- glm(MAS_~fundamentalism+antidemocr+
                                 desconfia+promercado,data = df, family=binomial(link=logit))
logitMAS__3 <- glm(MAS_~autoloc_dir_esq+interesse+
                                 fundamentalism+antidemocr+
                                 desconfia+promercado,data = df, family=binomial(link=logit))
logitMAS__4 <- glm(MAS_~X049+Gender+rend_subj+region+not_pertain_to_indigenous_group+
                                 idade_faixa+escolaridade_niveis+
                                 status_emplo+religion+autoloc_dir_esq+interesse+
                                 fundamentalism+antidemocr+
                                 desconfia+promercado,
                               data = df, family=binomial(link=logit))
tab_model(logitMAS__1,logitMAS__2,logitMAS__3,logitMAS__4,
...
bic_modelo1 <- BIC(logitMAS__1)
bic_modelo2 <- BIC(logitMAS__2)
bic_modelo3 <- BIC(logitMAS__3)
bic_modelo4 <- BIC(logitMAS__4)
...
# Definir "MAS_" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "MAS_")

library(nnet)
modeloMulti <- multinom(df_voto_multinom ~ X049+Gender+rend_subj+region+not_pertain_to_indigenous_group+
                          idade_faixa+escolaridade_niveis+
                          status_emplo+religion+autoloc_dir_esq+interesse+
                          fundamentalism+antidemocr+
                          desconfia+promercado,
...
logitfundamentalism_tercil <- glm(tercil_superior_fundamentalism~X049+Gender+rend_subj+region+not_pertain_to_indigenous_group+
                                    idade_faixa+escolaridade_niveis+
                                    status_emplo+religion+autoloc_dir_esq+interesse,
                                  data = df, family=binomial(link=logit))

logitdesconfia_tercil <- glm(tercil_superior_desconfia~X049+Gender+rend_subj+region+not_pertain_to_indigenous_group+
                               idade_faixa+escolaridade_niveis+
                               status_emplo+religion+autoloc_dir_esq+interesse,
                             data = df, family=binomial(link=logit))

logitpromercado_tercil <- glm(tercil_superior_promercado~X049+Gender+rend_subj+region+not_pertain_to_indigenous_group+
                                idade_faixa+escolaridade_niveis+
                                status_emplo+religion+autoloc_dir_esq+interesse,
                              data = df, family=binomial(link=logit))

logitantidemocr_tercil <- glm(tercil_superior_antidemocr~X049+Gender+rend_subj+region+not_pertain_to_indigenous_group+
                                idade_faixa+escolaridade_niveis+
                                status_emplo+religion+autoloc_dir_esq+interesse,
                              data = df, family=binomial(link=logit))
...
library(lsr)
df$MAS_ <- as.numeric(df$MAS_)
model<- aov(MAS_ ~ Gender+X049+not_pertain_to_indigenous_group+
              idade_faixa+escolaridade_niveis+
...
df$votos_partidos <- ifelse(dfao$E179WVS < 5, "Nenhum/NSabe",
                            ifelse(dfao$E179WVS == 68001, "MAS",
                                   ifelse(dfao$E179WVS == 68002, "Frente de Unid. Nac.",
                                          ifelse(dfao$E179WVS == 68003, "Movimiento Dem. Social",
                                                 ifelse(dfao$E179WVS == 68005, "Movimiento Nac. Rev.",
                                                        "Outros Partidos")))))

# Converter a coluna para o tipo "factor" com a ordem correta dos níveis
df$votos_partidos <- factor(df$votos_partidos,
                            levels = c("MAS", "Frente de Unid. Nac.",
                                       "Movimiento Nac. Rev.",
                                       "Movimiento Dem. Social", "Outros Partidos", "Nenhum/NSabe"))
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Bolívia Onda 7 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
