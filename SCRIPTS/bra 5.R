...
df <- read_csv("df.csv")
df <- df %>%
  filter(S002VS == 5 & S003==76)#brasil 5
...
summary(df[,1:13])
df[,14:28]#issues - ok, consta todos
...
df$votos_partidos <- factor(df$votos_partidos, levels = c("PSOL","PT", "PV", "PDT",
                                                          "PMDB", "PSDB", "PTB", "PFL",
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Brasil Onda 5 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
