...
df$PartidoSocialista <- df$E179WVS == 152007 
table(df$PartidoSocialista)
df$PartidoSocialista <- as.numeric(df$PartidoSocialista)
df$PartidoSocialista <- as.factor(df$PartidoSocialista)
table(df$PartidoSocialista)

# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 152007 , "PartidoSocialista",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("PartidoSocialista", "DK/NA/blank", "Others"))
...
df <- subset(df, select=c(raca_branc,Gender,idade_faixa,cidade_maior_bin,
                          escolaridade_niveis,status_emplo,rend_subj,religion,
                          autoloc_dir_esq, interesse,
                          PartidoSocialista,df_voto_multinom,
...
df <- subset(df, select=c(cidade_maior_bin,raca_branc,Gender,idade_faixa,
                          escolaridade_niveis,status_emplo,rend_subj,religion,
                          autoloc_dir_esq, interesse,
                          PartidoSocialista,df_voto_multinom,
...
prop.table(table(df$PartidoSocialista))
...
df <- subset(df, select=c(raca_branc,Gender,idade_faixa,cidade_maior_bin,
                          escolaridade_niveis,status_emplo,rend_subj,religion,
                          autoloc_dir_esq, interesse,PartidoSocialista,df_voto_multinom,
                          fundamentalism,desconfia,antidemocr))
rm(dfao,fit)
summary(df)
#modelos
logitPartidoSocialista_1 <- glm(PartidoSocialista~Gender+cidade_maior_bin+
                    idade_faixa+escolaridade_niveis+
                    status_emplo+rend_subj+religion,data = df, family=binomial(link=logit))
logitPartidoSocialista_2 <- glm(PartidoSocialista~fundamentalism+antidemocr+
                    desconfia,data = df, family=binomial(link=logit))
logitPartidoSocialista_3 <- glm(PartidoSocialista~autoloc_dir_esq+interesse+
                    fundamentalism+antidemocr+
                    desconfia,data = df, family=binomial(link=logit))
logitPartidoSocialista_4 <- glm(PartidoSocialista~Gender+cidade_maior_bin+
                    idade_faixa+escolaridade_niveis+
                    status_emplo+rend_subj+religion+autoloc_dir_esq+interesse+
                    fundamentalism+antidemocr+
                    desconfia,
                  data = df, family=binomial(link=logit))
tab_model(logitPartidoSocialista_1,logitPartidoSocialista_2,logitPartidoSocialista_3,logitPartidoSocialista_4,
...
bic_modelo1 <- BIC(logitPartidoSocialista_1)
bic_modelo2 <- BIC(logitPartidoSocialista_2)
bic_modelo3 <- BIC(logitPartidoSocialista_3)
bic_modelo4 <- BIC(logitPartidoSocialista_4)
...
# Definir "PartidoSocialista" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "PartidoSocialista")

library(nnet)
modeloMulti <- multinom(df_voto_multinom ~ Gender+cidade_maior_bin+
                          idade_faixa+escolaridade_niveis+
                          status_emplo+rend_subj+religion+autoloc_dir_esq+interesse+
                          fundamentalism+antidemocr+
                          desconfia,
...
logitfundamentalism_tercil <- glm(tercil_superior_fundamentalism~Gender+cidade_maior_bin+
                                    idade_faixa+escolaridade_niveis+
                                    status_emplo+rend_subj+religion+autoloc_dir_esq+interesse,
                                  data = df, family=binomial(link=logit))

logitdesconfia_tercil <- glm(tercil_superior_desconfia~Gender+cidade_maior_bin+
                               idade_faixa+escolaridade_niveis+
                               status_emplo+rend_subj+religion+autoloc_dir_esq+interesse,
                             data = df, family=binomial(link=logit))

logitantidemocr_tercil <- glm(tercil_superior_antidemocr~Gender+cidade_maior_bin+
                                idade_faixa+escolaridade_niveis+
                                status_emplo+rend_subj+religion+autoloc_dir_esq+interesse,
                              data = df, family=binomial(link=logit))
...
library(lsr)
df$PartidoSocialista <- as.numeric(df$PartidoSocialista)
model<- aov(PartidoSocialista ~ Gender+raca_branc+
              idade_faixa+escolaridade_niveis+
...
df$votos_partidos <- factor(df$votos_partidos, levels = c("Humanista", "Socialista", "PPD",
                                                          "DC", "RN", "UDI", "Outros Partidos",
                                                          "Nenhum/NSabe"))
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Chile Onda 5 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
