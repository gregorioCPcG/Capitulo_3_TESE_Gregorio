...
df$MORENA_ <- df$E179WVS == 484022
table(df$MORENA_)
df$MORENA_ <- as.numeric(df$MORENA_)
df$MORENA_ <- as.factor(df$MORENA_)
table(df$MORENA_)
prop.table(table(df$MORENA_))
# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 484022 , "MORENA_",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3,4),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("MORENA_", "DK/NA/blank", "Others"))
...
df <- subset(df, select=c(X049,raca_branc,Gender,idade_faixa,
                          escolaridade_niveis,status_emplo,rend_subj,religion,region,
                          autoloc_dir_esq, interesse,MORENA_,df_voto_multinom,
                          fundamentalism,desconfia,promercado,antidemocr))
rm(dfao,fit)
summary(df)
#modelos
logitMORENA__1 <- glm(MORENA_~Gender+X049+region+
                     idade_faixa+escolaridade_niveis+
                     status_emplo+rend_subj+religion,data = df, family=binomial(link=logit))
logitMORENA__2 <- glm(MORENA_~fundamentalism+promercado+antidemocr+
                     desconfia,data = df, family=binomial(link=logit))
logitMORENA__3 <- glm(MORENA_~autoloc_dir_esq+interesse+
                     fundamentalism+promercado+antidemocr+
                     desconfia,data = df, family=binomial(link=logit))
logitMORENA__4 <- glm(MORENA_~Gender+X049+region+
                     idade_faixa+escolaridade_niveis+
                     status_emplo+rend_subj+religion+autoloc_dir_esq+interesse+
                     fundamentalism+promercado+antidemocr+
                     desconfia,
                   data = df, family=binomial(link=logit))
tab_model(logitMORENA__1,logitMORENA__2,logitMORENA__3,logitMORENA__4,
...
# Definir "MORENA_" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "MORENA_")
...
library(lsr)
df$MORENA_ <- as.numeric(df$MORENA_)
model<- aov(MORENA_ ~ Gender+X049+raca_branc+
              idade_faixa+escolaridade_niveis+
...
df$votos_partidos <- ifelse(dfao$E179WVS < 5, "Nenhum/NSabe",
                            ifelse(dfao$E179WVS == 484022, "MORENA",
...
df$votos_partidos <- factor(df$votos_partidos, levels = c("MORENA", "PRD", "PRI", "PAN", "Outros Partidos", "Nenhum/NSabe"))
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos México Onda 7 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
