...
df <- read_csv("df.csv")
df <- df %>%
  filter(S002VS == 7 & S003==76)#bra onda _ 7
...
table(df$X047R_WVS)
#tratar como numeric
df$renda_subj_numeric <-as.numeric(df$X047R_WVS)
table(df$renda_subj_numeric, useNA = "always" )
df$renda_subj_numeric[df$renda_subj_numeric==-1] <- NA
df$renda_subj_numeric[df$renda_subj_numeric==-2] <- NA

table(df$renda_subj_numeric, useNA = "always" )#120 Na não vou perder vou imputar com mice]
...
df <- subset(df, select=c(X049,Gender,idade_faixa,region,raca_branc,
                          escolaridade_niveis,status_emplo,religion,
                          autoloc_dir_esq, interesse,
                          PT_,df_voto_multinom,renda_subj_numeric,
                          E069_07,E069_12,E069_17,E069_06,F028,
                          E037,E040,E036,E116,E114,E117,F120,F118,E039,E235))
summary(df[,1:12])
df[,13:28]#issues - ok, consta todos + renda subjetive numeric junto
summary(df[,13:28])#ver se tem que remover algo
# Atualizar valores menores que 0 por NA em cada coluna no intervalo 
for (i in 14:28) {
  df[, i][df[, i] < 0] <- NA
}#gera os NAS (menor q zero é NA no WVS)#nao no renda

# Visualizar o resultado
print(df[, 13:28])
summary(df[,13:28])
imp <- mice(df[,13:28], seed=23109)# o nome da base e a seed sempre essa 23109
df[,13:28] <- complete(imp, 1)#sempre a m como  destino da mputação , empre escolher a 1
rm(imp)
summary(df[,13:28])#renda + issues
...
df <- subset(df, select=c(X049,Gender,idade_faixa,region,raca_branc,renda_subj_numeric,
                          escolaridade_niveis,status_emplo,religion,
                          autoloc_dir_esq, interesse,
...
#modelos
logitPT__1 <- glm(PT_~X049+Gender+region+raca_branc+renda_subj_numeric+
...
logitPT__3 <- glm(PT_~autoloc_dir_esq+interesse+
                    ...
                    desconfia+promercado,data = df, family=binomial(link=logit))
logitPT__4 <- glm(PT_~X049+Gender+region+raca_branc+renda_subj_numeric+
                    idade_faixa+escolaridade_niveis+
...
library(nnet)
modeloMulti <- multinom(df_voto_multinom ~ X049+Gender+region+raca_branc+renda_subj_numeric+
                          idade_faixa+escolaridade_niveis+
                          status_emplo+religion+autoloc_dir_esq+interesse+
                          fundamentalism+antidemocr+
                          desconfia+promercado,
...
logitfundamentalism_tercil <- glm(tercil_superior_fundamentalism~X049+Gender+
                                    region+raca_branc+renda_subj_numeric+
                                    idade_faixa+escolaridade_niveis+
                                    status_emplo+religion+autoloc_dir_esq+interesse,
                                  data = df, family=binomial(link=logit))
...
logitpromercado_tercil <- glm(tercil_superior_promercado~X049+Gender+region+raca_branc+renda_subj_numeric+
                                idade_faixa+escolaridade_niveis+
                                status_emplo+religion+autoloc_dir_esq+interesse,
                              data = df, family=binomial(link=logit))

logitantidemocr_tercil <- glm(tercil_superior_antidemocr~X049+Gender+region+raca_branc+renda_subj_numeric+
                                idade_faixa+escolaridade_niveis+
                                status_emplo+religion+autoloc_dir_esq+interesse,
                              data = df, family=binomial(link=logit))
...
model<- aov(PT_ ~ Gender+X049+raca_branc+
              idade_faixa+escolaridade_niveis+
              status_emplo+renda_subj_numeric+religion, data= df)
etaSquared(model)
summary(model)
...
df$votos_partidos <- ifelse(dfao$E179WVS < 5, "Nenhum/NSabe",
                            ifelse(dfao$E179WVS == 76029, "PSOL",
                                   ifelse(dfao$E179WVS == 76002, "PT",
                                          ifelse(dfao$E179WVS == 76028, "REDE",
...
df$votos_partidos <- factor(df$votos_partidos,
                            levels = c("PSOL", "PT","REDE", "PMDB", "PSDB", "Outros Partidos", "Nenhum/NSabe"))
# Visualizar o resultado
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Brasil Onda 7 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
