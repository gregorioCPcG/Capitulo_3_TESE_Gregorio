...
#df$escolaridade_niveis <- ifelse(df$escolaridade_niveis == -1, NA, df$X025R)
...
print(dfao$X051)
table(df$X051)
df$halfblood_mestiz <- df$X051 == 558001
table(df$halfblood_mestiz)
...
df$FSLN_ <- df$E179WVS == 558001
table(df$FSLN_)
df$FSLN_ <- as.numeric(df$FSLN_)
df$FSLN_ <- as.factor(df$FSLN_)
table(df$FSLN_)
prop.table(table(df$FSLN_))
# Criar a coluna df_voto_multinom com base na coluna df$E179WVS
df$df_voto_multinom <- factor(ifelse(df$E179WVS == 558001 , "FSLN_",
                                     ifelse(df$E179WVS %in% c(-3, -2, -1, 2,3,4),
                                            "DK/NA/blank",
                                            "Others")),
                              levels = c("FSLN_", "DK/NA/blank", "Others"))
...
df <- subset(df, select=c(X049,halfblood_mestiz,Gender,idade_faixa,region,
                          escolaridade_niveis,status_emplo,rend_subj,religion,
                          autoloc_dir_esq, interesse,
                          FSLN_,df_voto_multinom,
...
df <- subset(df, select=c(X049,halfblood_mestiz,Gender,idade_faixa,
...
summary(fit)
#> ver 
#democracia abandons - contraintuitivo
...
#dfao$E117#ok
#scores[,3] -> df$antidemocr#abandons
scores[,4] -> df$promercado 
rm(scores,labels,model,values)

df <- subset(df, select=c(X049,halfblood_mestiz,Gender,idade_faixa,
                          escolaridade_niveis,status_emplo,rend_subj,religion,region,
                          autoloc_dir_esq, interesse,FSLN_,df_voto_multinom,
                          fundamentalism,desconfia,promercado))
rm(dfao,fit)
summary(df)
#modelos
logitFSLN__1 <- glm(FSLN_~Gender+X049+region+
                        idade_faixa+escolaridade_niveis+
                        status_emplo+rend_subj+religion,data = df, family=binomial(link=logit))
logitFSLN__2 <- glm(FSLN_~fundamentalism+promercado+
                        desconfia,data = df, family=binomial(link=logit))
logitFSLN__3 <- glm(FSLN_~autoloc_dir_esq+interesse+
                        fundamentalism+promercado+
                        desconfia,data = df, family=binomial(link=logit))
logitFSLN__4 <- glm(FSLN_~Gender+X049+region+
                        idade_faixa+escolaridade_niveis+
                        status_emplo+rend_subj+religion+autoloc_dir_esq+interesse+
                        fundamentalism+promercado+
                        desconfia,
                      data = df, family=binomial(link=logit))
tab_model(logitFSLN__1,logitFSLN__2,logitFSLN__3,logitFSLN__4,
...
# Definir "FSLN_" como a categoria de referência
df$df_voto_multinom <- relevel(df$df_voto_multinom, ref = "FSLN_")
...
logitpromercado_tercil <- glm(tercil_superior_promercado~X049+Gender+halfblood_mestiz+
...
library(lsr)
df$FSLN_ <- as.numeric(df$FSLN_)
model<- aov(FSLN_ ~ Gender+X049+halfblood_mestiz+
              idade_faixa+escolaridade_niveis+
...
df$votos_partidos <- ifelse(dfao$E179WVS < 5, "Nenhum/NSabe",
                            ifelse(dfao$E179WVS == 558001, "FSLN",
                                   ifelse(dfao$E179WVS == 558004, "Liberal Constitucionalista",
                                          "Outros Partidos")))

# Definir a ordem dos níveis na coluna "votos_partidos"
df$votos_partidos <- factor(df$votos_partidos, levels = c("FSLN", "Liberal Constitucionalista", "Outros Partidos", "Nenhum/NSabe"))
...
fundamentalismo<-fundamentalismo+labs(title="Prob. tercil superior de Fundamentalismo",
                                      x= "Partidos Nicarágua Onda 7 WVS",y="Probabilidade") + theme_minimal()
fundamentalismo
