#rodar bra7(em scripts primeiro)
library(poorman)
library(datawizard)
library(easystats)
...
library(ggplot2)
bra75<- data %>%
  filter(Partido == "PT" | Partido == "PMDB" | Partido == "PSDB" | Partido == "PSOL") %>% 
  ggplot(aes(
    x = name,
    y = value,
    color = Partido,
  ...
  scale_fill_manual(values = c("orange", "darkblue", "#fb6f92" ,"darkred")) +
  scale_color_manual(values = c("orange", "darkblue", "#fb6f92","darkred"))+ theme(legend.position = "none")


bra75
# se quiser salvar
saveRDS(bra75, file = "C:/Users/grego/OneDrive/Desktop/work/SCRIPTS TALK ABOUT/WVS/WVS confirmatoria/radar grafic/bra7.rds")
