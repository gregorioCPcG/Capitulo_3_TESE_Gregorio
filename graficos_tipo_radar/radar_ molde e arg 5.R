#rodar argentina 5(em scripts primeiro)
library(poorman)
library(datawizard)
library(easystats)
...
arg5<- data %>%
  ...
  coord_radar() +
  theme_radar() + labs(y="",x="") + facet_wrap(~Partido) + theme(
    axis.text = element_text(size = 7)  # Ajuste o tamanho da fonte para o valor desejado
  ) +  theme(
    axis.text.y = element_blank(),
    axis.title.y = element_blank()
  )  +
  scale_fill_manual(values = c("green", "#f77f00", "darkblue", "darkred")) +
  scale_color_manual(values = c("green", "#f77f00", "darkblue", "darkred"))+ theme(legend.position = "none")


arg5
# se quiser salvar
saveRDS(arg5, file = "C:/Users/grego/OneDrive/Desktop/work/SCRIPTS TALK ABOUT/WVS/WVS confirmatoria/radar grafic/arg5.rds")
