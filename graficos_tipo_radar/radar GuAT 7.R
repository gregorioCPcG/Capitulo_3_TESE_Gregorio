#rodar GUATEMALA_7(em scripts primeiro)
library(poorman)
library(datawizard)
library(easystats)
...
dfinho <- subset(df, select=c(desconfia,fundamentalism,votos_partidos,promercado))#nao tem PROMERCADO
dfinho %>% glimpse()
...
GUATEMALA_7<- data %>%
  ...
  scale_fill_manual(values = c("#6a994e","darkblue")) +
  scale_color_manual(values = c("#6a994e","darkblue"))+ theme(legend.position = "none")
