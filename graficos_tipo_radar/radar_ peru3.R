#rodar peru3(em scripts primeiro)
library(poorman)
library(datawizard)
library(easystats)
...
dfinho <- dfinho[,6:10]
dfinho %>% glimpse()
dfinho <- dfinho[, -4]#remover autoritario pq nao deu bons resultados na AFC
...
peru3<- data %>%
  ...
  scale_fill_manual(values = c("black","cyan","#6a994e","#fb6f92" )) +
  scale_color_manual(values = c("black","cyan","#6a994e","#fb6f92"))+ theme(legend.position = "none")


peru3
# se quiser salvar
saveRDS(peru3, file = "C:/Users/grego/OneDrive/Desktop/work/SCRIPTS TALK ABOUT/WVS/WVS confirmatoria/radar grafic/peru3.rds")
