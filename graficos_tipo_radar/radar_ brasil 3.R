#rodar bra3(em scripts primeiro)
library(poorman)
library(datawizard)
library(easystats)
...
library(ggplot2)
bra35<- data %>%
  filter(Partido == "PT" | Partido == "PMDB" | Partido == "PSDB" | Partido == "PFL") %>% 
  ggplot(aes(
    x = name,
    y = value,
    color = Partido,
  ...
  scale_fill_manual(values = c("black","orange", "darkblue" ,"darkred")) +
  scale_color_manual(values = c("black","orange", "darkblue","darkred"))+ theme(legend.position = "none")


bra35
# se quiser salvar
saveRDS(bra35, file = "C:/Users/grego/OneDrive/Desktop/work/SCRIPTS TALK ABOUT/WVS/WVS confirmatoria/radar grafic/bra3.rds")
