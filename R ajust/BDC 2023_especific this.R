...
df$Renda <- df$P60
df$semReligiao <- df$P13==9

df <- subset(df, select=c(P38,P40,P47,P37,P36,P14,P35,P39,P50,P51,P52,P53,P54,P55,P58,P17,
                          Renda,perfilProfissional,faixa_etaria,Mulher,Escolaridade,semReligiao))
...
#valors~partido
fundamentalism1 <- gglm(tercil_tercil_fundamentalism ~ P17_recod 
                      , data=df,family=binomial(link=logit))
summary(fundamentalism1)
# Calcule o pseudo R² de Nagelkerke usando o pacote fmsb
...
antidemocr1 <- glm(tercil_antidemocr~ P17_recod
                  , data=df,family=binomial(link=logit))
# Calcule o pseudo R² de Nagelkerke usando o pacote fmsb
...
# Exiba o resultado
print(nagelkerke)
promercado1 <- glm(tercil_promercado ~ P17_recod
                  , data=df,family=binomial(link=logit))
...
#valores ~estrutr
fundamentalism2 <- glm(tercil_fundamentalism ~ Mulher+
                        faixa_etaria+Escolaridade+
                        perfilProfissional+Renda+semReligiao 
                      , data=df,family=binomial(link=logit))
# Calcule o pseudo R² de Nagelkerke usando o pacote fmsb
...
antidemocr2 <- glm(tercil_antidemocr~ Mulher+
                    faixa_etaria+Escolaridade+
                    perfilProfissional+Renda+semReligiao
                  , data=df,family=binomial(link=logit))
# Calcule o pseudo R² de Nagelkerke usando o pacote fmsb
...
# Exiba o resultado
print(nagelkerke)
promercado2 <- glm(tercil_promercado ~ Mulher+
                    faixa_etaria+Escolaridade+
                    perfilProfissional+Renda+semReligiao
                  , data=df,family=binomial(link=logit))
